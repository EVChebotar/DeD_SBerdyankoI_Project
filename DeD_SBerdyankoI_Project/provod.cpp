...
void Provod :: organization(QDateTime RestartDateTime, QTableWidget *Table, QTimeEdit *AccumCurrentTime, QLineEdit *isPlusAccum, QTime AlertTime, QTimeEdit *UTC, QLineEdit *UTCPlusMinus){
    ...
    BufDateTimeForEtalon.setTime( ProvodEtalon );
    ...
    NewTimeEtalon.setTime( ProvodEtalon.addSecs( (UTCtoInt-3/* Т.к. эталоном считается время Мск (+3 UTC), то вычесть 3 */ ) *60 *60) );
    ...
    while (NewTimeEtalon < QDateTime :: currentDateTime()){
        NewTimeEtalon=NewTimeEtalon.addSecs(ProvodTime.second()+(ProvodTime.minute()+ProvodTime.hour()*60)*60);
    ...
    while(j<7){
        ...
        while( (NewTimeEtalon.date().day() != CheckDate) && (NewTimeEtalon.date().month() != CheckMonth) ){
            ...
            if ( (NewTimeEtalon.date().day() != CheckDate) && (NewTimeEtalon.date().month() != CheckMonth) ){
                ...
                i++;
                //qDebug() << "Рассчитанный тайм Везы Новая :" << timestr << endl;
                NewTimeEtalon=NewTimeEtalon.addSecs(ProvodTime.second()+(ProvodTime.minute()+ProvodTime.hour()*60)*60);
